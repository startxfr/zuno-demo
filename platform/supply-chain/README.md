...
Software supply chain policy: use immutable and verifiable software
supply chain artifacts.

`check_no_latest_tags.py` is the policy-as-code check for CI checks
rejecting `latest` for deployable component images: walks every
`gitops/charts/*/values.yaml` looking for an image `tag` set to the
literal `latest` (or left empty). No live cluster or registry needed -
pure static YAML inspection, same style as
...
environment to run it in. These values get bumped to the tags that
pipeline publishes via `pin_release.py` below - see `.github/README.md`
for what the workflow itself does.
...
`check_build_matrix.py` is the policy-as-code check that reconciles the
CI build inventory with the repository component lifecycle: validates
...
tracked first-party Dockerfile (e.g. `postgresql-pgvector` - Postgres is
now a Crunchy PGO-managed operand, not a first-party built image). Also
fails if a first-party `components/**/Dockerfile` exists with no matrix
entry at all.
