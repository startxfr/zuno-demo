...
Documentation reconciliation pipeline (ADR-0323: "Establish canonical
generated and validated platform documentation"). Treats ADRs and
executable configuration (Makefile, Ansible, Helm/Kustomize values, CR
...
release train, capability status, agent catalog) that was otherwise
duplicated - and drifting - across README.md, MEMORY.md,
`docs/architecture/*.md`, `docs/platform/*.md` and `platform/*/README.md`.
It deliberately does **not** capture dynamic OLM operator channel/catalog
selection (ADR-0048) - that stays runtime-discovered, never hard-coded.
...
- **make_commands** - every literal `make day0|d0|day1|d1 ...` example in
  README.md uses a verb/component the actual `Makefile` accepts (caught
  two real bugs on introduction: `make d0 configure` - `configure` isn't
  a Day 0 verb - and `make d1 run` - `run` isn't a Day 1 verb, both fixed
...
- **adr_index** - every `docs/adr/NNNN-*.md` file has a
  ...
  check). Roadmap container files (`0100-v1-roadmap.md`,
  `0200-v2-roadmap.md`, `0300-v3-roadmap.md`) are excluded - their
  embedded ADR-0101../0201../0301.. entries are indexed individually via
  anchor links, per the index's own existing convention.
...
- **version_consistency** - README.md/MEMORY.md/`docs/architecture/*.md`/
  ...
  target/release train. ADR bodies are immutable historical records and
  are excluded outright; RAG fixture/test data is excluded too - those
  are demo content, not platform documentation. A bare version number
  (e.g. "OpenShift AI 3.5") is treated as shorthand, not a contradiction,
  as long as it's a prefix of the declared release train ("3.5 EA2").
