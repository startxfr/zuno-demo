...
- `schema/003_rag_metadata.sql` - ADR-0046: extends
  `gitops/charts/rag-service/files/sql/002_pgvector.sql`'s `document_embeddings` table (owned
...
- `fixtures/seed.sql` - a fictional/synthetic demo corpus (same
  non-nominative-data spirit ADR-0025 requires)
...
Applied by `ansible/roles/rag` (after `ansible/roles/postgresql` has
created the base table) via the same one-shot `psql` Job pattern as
`ansible/roles/postgresql` itself - see that role's README.
