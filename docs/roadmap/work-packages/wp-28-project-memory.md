...
- **State:** Done (2026-08-23 - tracker correction: WP-31's 2026-08-15 merge already flipped ADR-0209 to Implemented and passed its cross-agent acceptance. Original note: 2026-08-15 — repo-provable, no operator step specific to this WP: `knowledge/project/domain.yaml` (mandatory `project_id`) + `knowledge/metadata-schema.yaml` entry; `rag-project` binding (`platform/bindings/knowledge/bindings.yaml`) riding WP-21's already-built pool registry with no special-casing needed; new PGO database/role, ExternalSecret and Vault seed (provisioning itself rides WP-21's operator step); `data/rag/schema/005_project_memory.sql` (`project_memberships` fail-closed membership table + `project_state`/memory-chunk tables), wired into the schema-apply Job. `policies/knowledge/knowledge-policy.yaml` gained a deliberately broad `knowledge.project` entry (every business-role group listed — membership, not group, is the real per-engagement gate, a sixth factor beyond ADR-0203's five, enforced by rag-service directly against `project_memberships`). Atomic BFF contract change (`openapi.json` + `main.go` + `internal/runtime/client.go`, `go build`/`go test` green throughout): optional `project_id` threads BFF → Agent Runtime. Agent Runtime: `AgentState.project_id` forwarded on every RAG call this turn makes; `retrieve_node` strips `knowledge.project` from the authorized set on any turn with no `project_id` (the knowledge-policy intersection may still authorize it — this is a separate per-turn gate on top); new `POST /v1/agents/tekos/runs/{run_id}/extract-memory` endpoint (explicit session-end trigger, no automatic per-turn extraction) reuses `_resolve_run_id`'s 404/403 ownership check and refuses (400) a run with no `project_id`; new `app/memory.py` extracts facts/decisions from the full checkpointer-reconstructed transcript (`_build_transcript`), forcing local-only inference for C2/C3 conversations per ADR-0035, classification inherited via `_escalate` (never re-derived); new `app/clients/project_memory_client.py` is the only path this runtime has to persist it — rag-service, not this runtime, holds the `knowledge.project` database credential. rag-service: `project_id`/`caller_sub` accepted on `SearchRequest`, a fail-closed `_check_project_membership()` (no row for the caller's sub or any of their groups = deny, before any content is touched) gates `knowledge.project` retrieval, and a new `POST /v1/project-memory/write` endpoint (`app/project_memory.py`) is the sole write path; the OGX provider has no SQL access to `project_memberships` and refuses `knowledge.project` outright rather than attempt an unverified check. 16 new tests: extraction fact/decision identification and ADR-0035 local-only forcing (`test_memory_extraction.py`, 8), membership fail-closed enforcement (`test_project_membership.py`, 6), and a single-runtime end-to-end acceptance test (`test_project_memory_e2e.py`) proving the exact Tekos `demo-001` scenario — a session states a fact, an explicit extraction call persists it via a mocked rag-service write, and a brand-new session/thread for the same project retrieves it back into its answer's citations without the user repeating themselves, plus a companion test that a run with no `project_id` is refused extraction. Full acceptance gate green: `lint_openapi.py`, `go build`/`go test`, both components' Python suites, `helm lint` (postgresql + rag-service), `check_knowledge_refs.py` + `check_docs.py`. The one remaining acceptance bullet — the cross-agent Arkos scenario — is out of scope for this WP by design and belongs to WP-31.)
