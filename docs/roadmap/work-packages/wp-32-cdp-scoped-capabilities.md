...
1. ~~Operator: re-apply the realm (`make d0 install keycloak` or the realm
   ...
   login.~~ Closed 2026-08-17: `KeycloakRealmImport/zuno-realm` was already
   `Done` on the live cluster (ArgoCD-synced from `origin/main`); verified
   via the Keycloak admin REST API rather than an interactive login — the
   `/cdp` group exists, `consultant-01`/`consultant-02` hold all four
   `confluence-archi-*` groups, `board-01`/`board-02` hold none of them.
