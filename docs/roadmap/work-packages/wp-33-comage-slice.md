...
- **Depends on:** WP-31 (merged — the slice template), WP-22 (its
  fixture-driven adapters, merged 2026-08-15 — not a live blocker), WP-23,
  WP-32, WP-24
...
1. ~~User: review the 20 scenarios.~~ Done 2026-08-30 (retroactive catch-up).
2. ~~Operator: Salesforce credentials in Vault..., deploy, run the
   acceptance gate at 75%, exercise one live freshness read + one write
   against the real (sandbox) Salesforce org~~ Gate run Done 2026-08-30
   (live PASS, 82%). The live Salesforce freshness read/write and the
   delegated Google Workspace action remain genuinely blocked - no
   Salesforce org has ever existed in this cluster (see the carve-out note
   below) - carved out to WP-101 rather than left as an unowned residual
   gap, the same pattern WP-090/ADR-0528 used today for its own Salesforce
   precondition.
3. ~~Operator: flip `agents/comage` to `active`~~ Already `active` since
   2026-08-27; this WP's remaining checkpoint (the gate proof) is what
   closed 2026-08-30.
...
- 2026-08-30, retroactive gate catch-up: tracker WP-33 → `Done`, closed on
  repo/live-verifiable criteria with the live Salesforce read/write pass
  carved out to WP-101 (v0.7) rather than gating this WP indefinitely on a
  precondition the repo cannot satisfy alone.

## Out of scope / deferred

- LoRA adapter for Comage (WP-34 — needs this bundle as training target).
- Advantage/Finage (WP-35/36).
- Salesforce MCP server install and the live freshness read/write pass:
  carved out 2026-08-30 to [WP-101](wp-101-salesforce-sandbox-credentials.md)
  (v0.7) rather than left as an unowned residual gap on this WP - no
  Salesforce org, sandbox or otherwise, has ever existed in this cluster's
  history (`ansible/roles/vault/tasks/install.yml` only seeds
  `zuno/salesforce/technical` when the operator supplies real, non-placeholder
  credentials, and none have been supplied). Comage's "Known live-read gap"
  note in `agents/comage/README.md` already documents the downstream
  chat-degradation behavior (source_mode stays indexed-only, never
  live/both). Resume once WP-101 provisions a real sandbox credential and
  re-adds the two `apply_gitops_app.yml` tasks removed from
  `ansible/roles/mcp/tasks/install.yml` on 2026-08-23.
