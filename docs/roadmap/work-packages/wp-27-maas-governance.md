...
- **State:** Done for the governance plane; agent-integration wiring deferred by scope
  ...
  `gateway`→workload) was investigated but left open — it doesn't block consumption.
  Known latent gap, upstream-owned: the generated AuthPolicy's `/llm/` path branch
  extracts model identities off by one (`llm/<ns>` instead of `<ns>/<model>`) — verified
  live 2026-08-25, harmless today because no `/llm/` route exists, tripwired in
  `ansible/roles/openshift_ai/tasks/precheck.yml`, upstream report ready in
  `docs/roadmap/evidence/maas-llm-path-identity-off-by-one.md`. Full
  evidence in ADR-0201's final 2026-08-25 notes.)
