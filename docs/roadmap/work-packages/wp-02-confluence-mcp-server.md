...
- **State:** Operator pending (2026-08-14 — repo work merged: real MCP server, binding registry wiring, policy entries, chart, build/deploy plumbing, protocol tests. Awaiting real Confluence Cloud credentials + e2e verification; ADR-0043's status-line follow-up is deferred with it, per this brief's Post-operator repo follow-up section. Re-verified 2026-08-17: repo acceptance checks re-run clean (`py_compile`, 6/6 protocol tests, `check_build_matrix.py`, `check_workload_hardening.py` 189/189, `helm lint`, handler-removed check, `check_docs.py`, atlassian-name grep). `confluence-mcp` is deployed live and Healthy in `zuno-ai-run`. Probed Confluence Cloud directly with the live `zuno/confluence/technical` credential (`cl@startx.fr`): `403 "Request rejected because caller cannot access Confluence"` from `startxfr.atlassian.net` - credentials resolve correctly, but the identity lacks a Confluence product-access grant on the Atlassian side. This is the same blocker the 2026-08-16 live-cluster session already recorded; still open, needs an Atlassian admin action outside this repo/session. `zuno-mcp-confluence-d1`'s ArgoCD status also shows `OutOfSync`/`Healthy` - root-caused to a cross-chart collision on the shared `mcp-gateway-workload-token` ExternalSecret (`mcp-gateway` renders it at `sync-wave: "10"`, `mcp-confluence`/`mcp-sales-db`/`mcp-salesforce` at `"25"`, so whichever Application syncs last leaves the others perpetually out of sync on that one annotation); not specific to WP-02, secret stays healthy, left unfixed as out of scope.)
