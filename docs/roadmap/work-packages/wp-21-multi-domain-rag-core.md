...
- **State:** Done (2026-08-15 — repo work merged: `platform/bindings/knowledge/bindings.yaml` + README, rag-service per-domain pool registry (`app/bindings.py`, `app/db.py`) with fan-out/merged-RRF search and ≥1-pool readyz, `knowledge.tech → rag-tech` repoint (schema `rag`, repo-root image build baking the bindings), three new PGO databases/roles (`rag-sales`/`rag-sxa-legacy`/`rag-adv`) + ExternalSecrets + init-sql GRANTs + Vault seeds, per-domain chart ExternalSecrets/env/schema-apply Jobs (new domains ship `enabled: false` until the operator provisions Vault + databases). Operator follow-up executed live 2026-08-15 (`sales.enabled: true` flipped in `gitops/charts/rag-service/values.yaml` with the GRANTs/schema-apply that made it real - see that file's dated comment) and re-verified 2026-08-17: all four domain databases/roles exist on the live PGO cluster (`ragtech`/`ragsales`/`ragsxalegacy`/`ragadv`, each ACL'd to only its own database); `rag-service` is deployed and Healthy; `tech`+`sales` schema-apply Jobs Complete, satisfying "two live domains with distinct credentials" (acceptance bullet 1). `make d1 check rag` was found reporting "NOT installed" despite this - `ansible/roles/rag/tasks/precheck.yml` still looked up a single hardcoded `zuno-rag-schema-apply` Job, a name that stopped existing the moment this WP switched to per-domain `zuno-rag-schema-apply-<domain>` Jobs; fixed to match on the `app.kubernetes.io/name=rag-service` label and require every returned Job to have succeeded. Re-ran `make d1 check rag` after the fix: reports "rag is installed". Repo acceptance re-run clean (rag-service pytest 54/54, helm lint, helm template renders the three new db/role blocks with `postgresCluster.enabled=true`, `check_knowledge_refs.py`, physical-name grep). ADR-0204 itself stays Partially implemented - it needs WP-22 (source adapters' live runs, not in this WP's scope) before it can flip.)
