...
- Finage (WP-36).
- `knowledge.adv`'s data source. ADR-0218 (2026-08-26) removed
  `fetch-aramis` and the domain has no ingestion adapter of any kind; the
  descriptor, policy entry and `rag-adv` binding are retained. Finding a
  replacement source or accepting the gap is a separate decision for
  whoever owns this slice, and needs its own ADR — it is explicitly not
  an operator credential action any more.
