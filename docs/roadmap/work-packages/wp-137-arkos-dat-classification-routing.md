...
- **State:** Done (2026-09-05 - repo-side mechanism complete: DAT baseline is project-derived (C1 default), reflect_node follows effective_classification instead of a fixed C2 ceiling, ai-gateway's new `local_only_for` policy field fail-closes C2/C3 to local providers, and the OKF authorization matrix regenerates clean. Automated tests 1, 2 (implied by 1's C1 code path), 3, 4, 5, 8, 9, 10 pass; tests 6/7 exercise pre-existing, unmodified mechanisms (`local_only_required`, provider-failure fallback) already covered elsewhere in the suite. The live C1/C2 comparison ran clean 2026-09-05 during WP-136's rehearsal 1/2: no-project DAT request -> `effective_classification=C1, provider=ovhcloud-gpt-oss-120b, execution=external, fallback=no`; `webinar-confidential` (C2) project, same prompt -> `effective_classification=C2, provider=local-gpt-oss-maas, execution=local`, GPU-backed local serving confirmed in OpenShift AI Dashboard/Console. Four distinct live-only defects surfaced and were fixed getting to that clean result - see items 2/2b/2c/2d below.)
- **ADRs:** ADR-0550
...
2b. **A second, compounding finding, same rehearsal session**: even after the fix above, WP-136's own suggested Step 1 prompt ("Draft a one-paragraph architecture summary for a public reference case about X") still came back C2, because it does not match any of `_extract_topic`'s `_TOPIC_PATTERN` forms in `arkos_nodes.py` (which require "dat"/"architecture testimonial"/"document" as the object noun - "architecture **summary**" doesn't match). `plan_node` then falls back to the WHOLE raw sentence as the Confluence search topic, and the generic framing words ("architecture", "summary", "reference", "case") matched real pages in this cluster's live Confluence instance regardless of the actual subject - confirmed by testing a deliberately unrelated topic ("renewable energy grid modernization") with the same boilerplate framing, which also came back C2. Fixed in `ansible/playbooks/demo_step_1.yml`'s suggested prompt ("Draft an architecture testimonial about X.", which does match `_TOPIC_PATTERN` and extracts a clean topic) - no agent-runtime code change needed for this half, since the extraction regex was already correct, only WP-136's own prompt wording was wrong.
2c. **A third finding, same rehearsal session, this time in `confluence.page.search` itself** (`components/mcp-servers/confluence/server.py`): even with a clean topic extraction, THREE deliberately unrelated topics in a row (container platforms, a hospital's EHR migration, renewable energy) all still came back C2 - confirmed live that `confluence.page.search` genuinely returned non-empty `results` each time. Root cause: Confluence Cloud's own CQL `text~` ranking behaves closer to a fuzzy per-word OR match than an AND/phrase match, so any query sharing even one common IT word ("cloud", "migration", "platform"...) with a real page counts as a hit, regardless of true topical relevance - and ADR-0034 escalates the moment this tool returns ANY result. Fixed with a post-filter (`_is_relevant_result`, new `_significant_words` helper) requiring at least half of the query's own significant words to appear in the hit's title before counting it as relevant - calibrated against the one real, live-verified match this server already relies on (ADR-0330's "version 4.14" match, 3 of 5 significant words in the title) so that known-good scenario still passes. Covered by `test_is_relevant_result_requires_majority_of_significant_words_in_title`/`test_search_pages_filters_out_a_cql_hit_that_does_not_match_the_title` in `components/mcp-servers/confluence/tests/test_mcp_protocol.py` (9/9 pass). This changes a shared MCP server every Confluence-search-capable agent uses, not just Arkos - re-verify Tekos's own live Confluence scenario still finds its target page after this change, alongside re-verifying Step 1's clean C1.
2d. **A fourth finding, same rehearsal session, this time in the deployment process itself, not the code**: after fixing 2c, the live C1 prompt still came back C2/local-only with the raw, pre-fix `retrieve_node` behavior clearly visible in the running pod's own source (`oc exec ... sed -n` on `/app/app/graph/arkos_nodes.py`). Root cause: commits `0831a8e6` (this WP's own fix) and, separately, the confluence-mcp CQL fix's commit had been made locally but the `agent-runtime`/`mcp-confluence` rebuilds that were supposed to deploy them ran `make d2 build ...`, whose BuildConfig clones `origin/main` - and the confluence-mcp fix commit was still sitting unpushed (`git status`: "en avance de 2 commits") when its rebuild ran, so the resulting image was byte-identical to the pre-fix one despite the build reporting `Complete`. `git push origin main` followed by a fresh `make d2 build agent`/`make d2 build mcp` (confirmed this time via `oc get builds ... -o jsonpath='{.spec.revision.git.commit}'` matching the intended commit, not just "Build succeeded") resolved it - live-verified: a fresh Arkos conversation with no project now returns `effective_classification=C1, provider=ovhcloud-gpt-oss-120b, execution_location=external, local_only_required=false` for the WP-136 Step 1 prompt. This is the existing "push before in-cluster build" trap (BuildConfig clones origin/main, not local) recurring - the actionable lesson is to verify the Build's *actual* git commit after every "successful" rebuild during this kind of rapid live-iteration, not just its Complete/exit-0 status.
3. **Done 2026-09-05**: ran the "Live verification" scenario above (real frontend/BFF, the three demo projects, C1 and C2 DAT prompts) and recorded effective classification, selected provider/model, and local-serving evidence for C2 - see the State line above for the clean result.
4. This WP's tracker is now `Done`; contributes toward ADR-0550 `Status` -> `Implemented` once WP-136 is verified too (WP-135 already verified alongside this same rehearsal).
