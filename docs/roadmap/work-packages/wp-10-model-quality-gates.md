...
- 2026-08-22: executed as above, with one correction - the LMEvalJob chart
  moved from `gitops/charts/openshift-ai/` to `gitops/charts/models/`
  (commit `2e25557`, predates this closure) before this closure landed, so
  ADR-0108's `Implemented` note points at `gitops/charts/models/` and
  `evaluations/benchmark.py` instead. ADR-0108's own gap (not this WP's
  earlier steps) needed a real live fix, not just a status flip - see its
  2026-08-22 dated note.
