...
Also retargeted here 2026-08-30: ADR-0307 (self-service agent onboarding) and ADR-0410 (expand the agent catalog beyond the initial five agents), both `Proposed`. WP-41, which would have discharged them, was Cancelled 2026-08-23 - the merged template generator (`platform/templates/agent/`) and Naveo agent bundle (`agents/naveo/`) stay in the repo, but the sixth-agent deployment gate is not being pursued. They move here from v0.4 rather than staying open against a milestone that is otherwise fully closed.
