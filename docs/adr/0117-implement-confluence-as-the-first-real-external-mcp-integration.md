...
- **Status:** Implemented - see `components/mcp-servers/confluence/`. Live e2e verified 2026-08-18 (roadmap WP-02) after the Atlassian product-access grant landed (the 2026-08-17 `403` is resolved): `confluence.page.search` and `confluence.page.read` executed through the gateway against real `startxfr.atlassian.net` content (caller `consultant-01`, tekos/arkos task declarations); `confluence.page.create` through the gateway correctly denied 403 by the ADR-0011 agent-declaration intersection (no agent declares write capabilities today); all four provider tools verified against the live Confluence REST API via the `service-identity` binding, with `create_page`/`update_page` exercised in the technical identity's own personal space (page created v1, updated v2, deleted after the run); the indexed `knowledge.tech` read leg returned real chunks from `rag-service /v1/search`. Pre-existing side-note, unchanged and out of this ADR's scope: `zuno-mcp-confluence-d1` shows perpetual ArgoCD `OutOfSync`/`Healthy` from the shared `mcp-gateway-workload-token` `ExternalSecret` sync-wave collision (recorded 2026-08-17).
- **Target:** v0.1
- **Date:** 2026-08-13
