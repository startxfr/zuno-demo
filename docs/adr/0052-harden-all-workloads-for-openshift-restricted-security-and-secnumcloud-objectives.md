...
- NetworkPolicies: `gitops/charts/namespaces` gained a `platformNamespaces` baseline (default-deny-other-namespaces + specific known cross-namespace allows) for `zuno-auth`/`zuno-data`/`zuno-monitoring`. `zuno-ai` deliberately does NOT get this baseline (see ADR-0037's implementation note: a broad same-namespace allow would defeat sales-db-mcp's isolation) - every `zuno-ai` workload instead has its own precise, workload-owned `NetworkPolicy`.
