...
- **Status:** Partially implemented (DSC migration, health checks, OGX provider and parity tests merged and live; `ogxServer.enabled` flipped true 2026-08-18 (roadmap WP-06) - the `OGXServer` CR now creates successfully after fixing real schema drift (`distribution.name: rh`, not `remote-vllm`), but its reconciliation is blocked on a genuine RHOAI 3.5 EA2 operator limitation: the OGX operator's own in-process OCI-manifest fetch to `registry.redhat.io` 401s despite valid cluster and linked ServiceAccount pull credentials - `imagePullSecrets` govern kubelet pulls, not the operator's own registry client, and this build documents no alternative wiring. Corpus proof and live provider-parity run stay blocked on that upstream gap, not a repo gap; re-attempt once the operator's registry-credential mechanism is documented or fixed upstream.)
- **Target:** v0.1
- **Date:** 2026-08-11
