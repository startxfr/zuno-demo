...
**What remains for WP-27** is therefore an RHOAI-side question, not a repo change: either MaaS must accept the adopted route's identity form when resolving a subscription, or it must expose a way to declare the published identity (an `endpointOverride`-style field for the model key). A probable upstream bug in the `/llm/` path form's identity extraction was also spotted here; it has since been **verified live** — see the dedicated 2026-08-25 note near the end of this ADR ("the /llm/ path branch extracts identities off by one").
...
## Implementation note (2026-08-25, addendum) — verified: the `/llm/` path branch extracts identities off by one; latent, upstream-owned, tripwired

The earlier "probable upstream bug" is now confirmed by live test, and precisely bounded. It does **not** reopen WP-27 — the resolved flow above never enters this branch.

**The defect.** `maas-gateway-auth` (generated by `maas-controller`, `generation=1`, untouched by anything in this repo) derives the model identity from the request path whenever `request.path.startsWith("/llm/")`. The extraction logic appears **six times** — five CEL expressions (the `subscription-info` metadata body's `requestedModel`, three authorization cache-key selectors, and the `selected_subscription_key` response filter) plus, independently, the OPA Rego's `path_model_identity` — and all six index the same way:

```
filtered := [segment for segment in path.split("/") if segment != ""]
identity := filtered[0] + "/" + filtered[1]
```

For `/llm/<ns>/<model>/v1/...` the filtered segments are `["llm", "<ns>", "<model>", "v1", ...]`, so the computed identity is `llm/<ns>`. Verified live against maas-api's own selector: `requestedModel: "llm/zuno-ai-run"` → `not_found` / "no matching subscription found for user"; `"zuno-ai-run/gpt-oss-20b-maas"` (what `[1]/[2]` would produce) → the full `gpt-oss-20b-tekos` subscription. The guard and the indices disagree by exactly one segment: `[0]/[1]` **would** be correct for the bare `/<ns>/<model>/...` form — which is exactly the form the guard does *not* select (bare paths fall through to the `x-gateway-model-name` header branch, which is how the RESOLVED flow works). Two self-consistent upstream fixes exist: shift the indices to `[1]/[2]`, or drop the `/llm/` prefix from the guard so the bare form takes the path branch.

**Why it is latent here.** No HTTPRoute on this cluster matches `/llm/` — the branch is dead code today. Any request to an `/llm/...` path 404s at routing before auth ever evaluates. If RHOAI (or anything else) ever publishes an `/llm/`-prefixed route on `maas-default-gateway`, every request through it will 403 with "no matching subscription found for user" regardless of entitlement — the same failure signature the header form had before the alias fix, which is why it was worth pinning down now rather than rediscovering it mid-demo.

**Why it is not fixable from this repo.** The expressions are controller-generated (this repo authors only the high-level `MaaSAuthPolicy` CR); a live patch fights the controller, and hosting a corrected route-level override on a hand-authored `/llm/` route lands in the already-eliminated trap of attempt 1 (`gateway-default-deny`, 429 on everything). What this repo does instead: a precheck tripwire in `ansible/roles/openshift_ai/tasks/precheck.yml` fires a blocked-finding the moment an `/llm/` route appears while the live AuthPolicy still carries `[0]`-indexed extraction, and the full reproduction for an upstream report lives in `docs/roadmap/evidence/maas-llm-path-identity-off-by-one.md` (RHOAI `3.5.0-ea.2`, CSV `rhods-operator.3.5.0-ea.2`).
