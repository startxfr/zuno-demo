...
- **Status:** Partially implemented - the governance plane is live and enforcing, and the one remaining gap is an RHOAI-side constraint rather than this repository's configuration. Auth IS attached to live model requests (16 wasm actionSets on `maas-default-gateway`, 8 of them path-prefix-keyed and needing no injected header; an unauthenticated path-prefixed completion returns 401), the ipp ext_proc filters are re-anchored and working, and the entitlement plane resolves correctly when queried directly. What blocks the acceptance bullets is a model-identity mismatch: MaaS mandates that it adopt KServe's HTTPRoute, so the served identity is always `publishers/...`, while maas-api and both generated policies key on `<modelRef namespace>/<modelRef name>`. Three local fixes were tried and eliminated by live test - see the 2026-08-25 notes below.
