...
The `zuno-ai-run`/`zuno-ai-build` split narrows the blast radius of a compromised build (build-time supply-chain risk, ADR-0115) so it cannot directly reach running workload namespaces. Cross-namespace image pulls from `zuno-ai-build` into `zuno-ai-run` are granted via a scoped `system:image-puller` RoleBinding, not a broader namespace-wide trust relationship. AdminContext's PriorityClasses/StorageClass check/ClusterRoleBinding consolidation must not silently grant broader cluster-admin-equivalent access than the single existing ArgoCD application-controller binding already requires.
