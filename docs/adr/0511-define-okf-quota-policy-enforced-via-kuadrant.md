...
- **Status:** Implemented - see `policies/quotas/` and `platform/okf/generate_quota_enforcement.py`. The 429-exceedance acceptance run passed live 2026-08-25 with a real token, after that run found and fixed three stacked defects that had left enforcement counting nothing at all — see the 2026-08-25 note below.
...
## Implementation note (2026-08-25) — the acceptance run was not a formality: three stacked defects had left enforcement inert

The 2026-08-24 note below closes by calling the remaining 429-exceedance demo "a normal acceptance run... not blocked by any defect". Running it disproved that. The first pass sent 86 authenticated requests through `zuno-agent-gateway` and got a clean `200` on every one — no `429` anywhere, and no error surfaced by any component. Three separate defects were in the way, each hidden behind the previous one. All three are fixed and the run now passes.

**1. The AuthPolicy never published the identity the counters read.** Authorino authenticated each request and returned OK, but an identity only reaches the wasm-shim as ext_authz *dynamic metadata*, which the AuthPolicy must ask for. Ours did not, so `auth.identity.sub` and `auth.identity.groups` were unresolvable and the shim skipped the rate-limit call entirely. Evidence: one line per request on the gateway proxy, `kuadrant_wasm_shim: Failed to evaluate message builder: CelError::Resolve { NoSuchKey("identity") }`, with `kuadrant-auth-service` at `rq_total 90` while `kuadrant-ratelimit-service` sat at `cx_total 0 / rq_total 0`. RHOAI's own `maas-gateway-auth` AuthPolicy publishes an identically-named `identity` filter, which is why MaaS's token limits worked on the sibling gateway and this one never could. Fixed by adding `response.success.filters.identity` to `templates/quota-demo-route.yaml`.

**2. Class predicates failed whenever the header was absent.** With identity flowing, the `intensive` class limited correctly at exactly its 10/5m boundary — and `standard` still never limited. CEL specifies that a logical operator absorbs an error in one operand when the other decides the result, so `!('x-zuno-quota-class' in request.headers) || request.headers['x-zuno-quota-class'] == 'standard'` should be plain `true` for a request carrying no such header. This shim's evaluator does not absorb it (`CelError::Resolve { NoSuchKey("x-zuno-quota-class") }`), and one failed expression fails the whole message builder — so *no* descriptor was sent and every limit went uncounted, not only the one whose predicate broke. That struck the `standard` class precisely because "header absent ⇒ standard" is this policy file's documented default: the class covering ordinary traffic was the one that could never be enforced.

**3. Kuadrant concatenates every limit's predicate with `||`, and `?:` binds looser.** Replacing the guarded disjunction with a conditional was correct in isolation and still produced no `429`. The shim's next error printed both the mangled AST and the expression it had actually compiled: Kuadrant does not evaluate each limit's predicate separately, it joins them all into one `||` chain deciding whether the actionSet applies. An unparenthesized ternary does not survive that — `A ? B : false || C ? D : true` parses as `A ? B : ((false || C) ? D : true)`. Each predicate now carries its own outer parentheses, so every term is a self-contained boolean regardless of how many limits the policy file grows.

**The shared lesson, and why a harness layer now exists.** Every one of these fails silently and completely: the `RateLimitPolicy` reports `Accepted`+`Enforced`, all six limits stay compiled in Limitador, requests return a clean `200`, and nothing anywhere logs a problem except the gateway proxy's own wasm lines. `Accepted`/`Enforced` and the presence of compiled limits are therefore *not* evidence of enforcement — only driving traffic past a threshold distinguishes "enforcing" from "silently inert". That is why the run is now `platform/testing/quota_429.py`, wired into `day2_stresstest.py` as the `quota` layer rather than left as an operator command, and why `generate_quota_enforcement.py` now asserts that every `auth.identity.<name>` its generated counters dereference is actually published by the AuthPolicy.

**Acceptance evidence (2026-08-25, `consultant-01`, real Keycloak token):** unauthenticated request `401`; `intensive` first `429` at request 11/18 against a 10/5m limit; `standard` first `429` at request 61/68 against a 60/5m limit; the two thresholds differing is the per-class counter-keying proof; zero `5xx` across 86 requests. The compiled limits appear in Limitador as `standard-user`/`standard-group`/`standard-project` and their `intensive-` counterparts under namespace `zuno-ai-run/tekos-quota-demo` — not under a `zuno-quota-*` prefix, which the chart README previously told operators to look for.
