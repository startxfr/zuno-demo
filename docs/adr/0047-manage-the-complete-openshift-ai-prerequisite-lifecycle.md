...
- Per-capability disposition: NFD - genuinely needed (new role). GPU Operator - already had a role, now correctly ordered after `nfd`. cert-manager/Service Mesh/Serverless - not installed, the RawDeployment fix removes the need for all three. Connectivity Link - not applicable, this project's own MCP Gateway/AI Inference Gateway (ADR-0010/ADR-0009) are its policy enforcement points. LeaderWorkerSet - not applicable, one always-on single-GPU model, no multi-node/multi-GPU topology. MaaS - not applicable to v0, deferred to v1 (ADR-0114). OGX - not a separate operator, ADR-0018 defines it as this project's name for capabilities (`kserve`, RAG) already covered above.
