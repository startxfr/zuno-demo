...
- **Status:** Superseded in part by [ADR-0528](0528-rekey-project-binding-quota-and-telemetry-onto-the-zuno-project-id.md) (2026-08-27) - clause 3 (where and when the Salesforce binding is verified, and what `conversations.project_id` holds) and clause 4's quota/observability keying are replaced: the Salesforce opportunity becomes an optional attribute of an ADR-0527 project, verified at project create/update instead of at conversation start, and `X-Zuno-Project-Id` carries the Zuno `project_id` for every project, customer or free. Clauses 1 and 2 - the `zuno.project_required` frontmatter mark and prompt-side collection - stand unchanged, as does clause 4's requirement that a verified binding activate the project's quota and scope `knowledge.project` retrieval. Prior status: Partially implemented (schema, prompts, runtime binding and scoping merged by WP-55; live Salesforce verification never performed, blocked on the standing WP-22/WP-33 sandbox credential gap and on `finance` missing from `salesforce.opportunity.read`'s `allowed_groups`).
