...
| Every physical tool binding declares an explicit, non-inferred authentication mode | `enforced-in-ci` (2026-08-15); `enforced-on-cluster` (2026-08-26, partial) | `platform/bindings/tools/tool-bindings.yaml` (ADR-0208); enforced in `main.py`'s `invoke_tool` between the policy decision and `invoke_downstream`, `app/delegation.py`; `test_auth_mode_enforcement.py` (WP-26). First live proof 2026-08-26: an authenticated `consultant-01` call to `list_drive_files` (`delegated-user`) with no delegated credential got a live `403 delegated_credential_missing` from the running mcp-gateway. The `provider-delegated` branch (`workday.profile.*`) could not be exercised live the same way - no agent declares a Workday tool in any `agent.okf.md` today, so the call is denied earlier, at the agent-declaration check (ADR-0011), before `invoke_tool` ever reaches the auth_mode dispatch; that branch stays `enforced-in-ci`-only (correct code, currently unreachable in the live system) until an agent adopts it |
...
| Platform namespaces (`zuno-auth`, `zuno-vault`, `zuno-data`, `zuno-monitoring`, `zuno-ai-build`, `zuno-mesh`) get a default-deny-other-namespaces baseline with explicit allow-lists | `enforced-in-ci` | `gitops/charts/namespaces/templates/networkpolicy-platform.yaml` |
...
| A restore has actually been executed and RTO/RPO validated live | `enforced-on-cluster` (2026-08-18, re-verified 2026-08-25) | `docs/platform/backup-recovery.md`'s drill records (ADR-0112 / WP-13): PostgreSQL scratch-cluster restore Ready in 203s, data verified identical to the live primary; Vault snapshot restore unsealed with the live key, known secret verified at 39s. Both well inside RTO ≤ 4h. Re-run 2026-08-25: PostgreSQL scratch restore Ready in ~330s (a smaller-footprint scratch cluster than the first run), `rag.document_embeddings` matched the live primary exactly (68,944 rows, identical `max(created_at)`); Vault restored from a fresh snapshot into a scratch pod, unsealed with the live key, known secret matched at ~64s. Both scratch environments deleted after |
...
| A failover drill has been executed against the shared platform services | `enforced-on-cluster` (2026-08-18, re-verified 2026-08-25/26) | `docs/platform/slo.md`'s drill table (ADR-0101 / WP-12): PostgreSQL primary failover 4.8s/5.7s-writable; scaled rag-service continuity 79/81 requests through a pod kill with the PDB holding; single-replica services recover in 31-65s. Re-run 2026-08-25/26: PostgreSQL primary failover 24.9s Ready / 45.0s writable; scaled rag-service continuity **40/40** requests through a pod kill, PDB held; single-replica recovery mcp-gateway 32.8s, ai-gateway 34.9s, agent-runtime 34.9s (fresh restart also re-verified all 8 OKF signatures cleanly), redis 59.2s, keycloak 55.7s (OIDC endpoint 200 immediately after) |
