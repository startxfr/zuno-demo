...
- Work is decomposed into self-contained work-package briefs under `docs/roadmap/work-packages/`, all tracked in the single `docs/roadmap/implementation-roadmap.md` (renamed from `v0.1-v0.3-implementation-roadmap.md` on 2026-09-03; the OKF stream's three phases folded in as phases 33-35 and `okf-roadmap.md`'s tracker became a pointer), each written for standalone execution by a lower-capability model. Phase headings do not assert a version - the derived "Work packages by version" table carries that axis. Zero padding is significant: `WP-57`/`WP-057` and `WP-58`/`WP-058` are four different work packages.
...
- The ADR index is grouped by `Target` and carries no `Target` column in its active sections - the section heading *is* the target. Status is a closed vocabulary: `Proposed | Accepted | Partially implemented | Implemented | Deferred | Deprecated | Superseded by ADR-NNNN | Superseded in part by ADR-NNNN`. Qualifications go in the ADR body, never in the index cell.
- A decision changes in one of three ways, and picking the wrong one is what let the index rot: **full supersession** (record moves to the index's `Retired` section), **partial supersession** (`Superseded in part by`, record *stays* in its version band because part of it is still live architecture), or a **dated correction note with no supersession** (records contradict on a detail, every decision clause still stands - ADR-0518/0526/0531 on the fleet default). Clauses that migrate *into* the successor are a full supersession, not a partial one (ADR-0213 -> ADR-0527).
- No ADR numbering band is reserved for any stream. The 05xx-band-for-OKF convention was retired 2026-09-03 (34 of its 42 records were not OKF). ADR-0541 is free; published ADRs and WPs are never renumbered to close a cosmetic gap.
- `check_docs.py` grew from validating one pair (index Status vs body Status) to also checking `Target`, section placement, the status vocabulary, index-row integrity (duplicate/dangling/unmarked-stub rows), declared supersede reciprocity, and the roadmap's by-version table. It reported PASS for weeks over 33 mis-sectioned rows and nine unpropagated supersessions; the lesson is that a green validator only covers what it actually inspects.
