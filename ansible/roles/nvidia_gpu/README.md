...
two L4 worker nodes. A Day 1 component (ADR-0056; moved here from Day 0 by
ADR-0421) - `install.yml` applies the whole chart, in two
`apply_gitops_app.yml` calls (see below).
...
`ansible/playbooks/day1_{check,install}.yml` list `nfd` immediately before
`nvidia_gpu` in `day1_components` accordingly (both moved from Day 0 to
Day 1 by ADR-0421; `machines`, which the default `ClusterPolicy` also
implicitly needs a GPU node from, stays in Day 0 and still runs first).
