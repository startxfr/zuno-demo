...
depends on (ADR-0056). Runs right after `argocd` in the Day 0 sequence
...
own ArgoCD Applications (ADR-0314).

Since ADR-0314, this role is a standard `-d0`/`-d1` Application pair like
every other Day 0 component (ADR-0312), backed by `gitops/charts/
admin-context`:
...
- `install.yml` - verifies at least one `StorageClass` exists (discover-
  ...
  clear diagnostic if none does), applies both Applications via
  `ansible/tasks/apply_gitops_app.yml`, and reports on the
  ...
  grant. This binding is always already present by the time this role
  runs, since `argocd` precedes it in the Day 0 sequence.
