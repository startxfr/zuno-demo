...
other-namespaces `NetworkPolicy` (ADR-0052) to each, with a small, explicit
list of known real cross-namespace callers. `zuno-ai-run` is deliberately
excluded from that baseline - see `values.yaml`'s comment on why ADR-0037
needs per-workload NetworkPolicies there instead of a namespace-wide
baseline.

ADR-0329 (supersedes ADR-0023) retired the per-agent `zuno-agent-<name>`
namespace model this chart used to create: agent workloads no longer get
their own namespace, quota or NetworkPolicy baseline here - every active
agent's FE/BFF deploys into the shared `zuno-ai-run` namespace instead
(`gitops/charts/tekos`).
...
`agents/<name>/agent.okf.md` declarative bundles - per ADR-0007, four of
the five agents are only declaratively defined in v0 - with no dedicated
