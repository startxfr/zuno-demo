...
| `aap` | local chart, `gitops/charts/aap` (ADR-0354 - `-d0`: startx `operator` dependency for the AAP `Subscription`/`OperatorGroup`, subscribed into its own `zuno-aap` namespace same as `keycloak`; `-d1`: the unified `AnsibleAutomationPlatform` CR (Gateway/Controller/Hub/EDA) + four per-sub-component `ExternalSecret`s - see the `aap` role's README) |
| `aap-config` | local chart, `gitops/charts/aap-config` (ADR-0354 clause 4/WP-073 - `-d0` is a no-op, the `tower.ansible.com` CRDs come from `aap`'s own operator bundle; `-d1`: the `zuno-demo` `AnsibleProject`, `zuno-day0-check` `JobTemplate`, their inventory/credential CRs and the cluster-reader ServiceAccount - the org/token/SSO half is API-driven by the `aap_config` role, see its README) |
