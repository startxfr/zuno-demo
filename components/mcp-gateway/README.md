...
## Sales-db MCP server: real standards-compliant MCP protocol

`components/mcp-servers/sales-db` speaks a real, standards-compliant MCP
server (the official `mcp` Python SDK, streamable-HTTP transport, mounted
at `/mcp`). `app/downstream.py:_invoke_streamable_http` uses the SDK's
own `mcp.client.session.ClientSession` +
...
gateway's own policy intersection already happened). Its endpoint comes
from the sales-db entries' `endpoint` block in the binding registry.
...
`{"customer": ..., "contacts": ...}` shape. See
`tests/test_downstream_sales_db.py` for a local MCP server fixture that
...
since agent-runtime and sales-db-mcp share the `zuno-ai-run` namespace and
a direct agent-runtime-to-MCP-server path must be forbidden.
