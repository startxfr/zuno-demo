...
(one frontend deployment per agent, same shared codebase):

1. **The agent portal** (`GET /`) - one tile per agent, read at startup from
   every `agents/<name>/agent.okf.md` OKF v0.2 Markdown bundle baked into
   the image (see `Dockerfile`). A tile is clickable only if the agent's
   `zuno.status` is `active` **and** the signed-in user's JWT `groups`
   claim intersects the agent's `zuno.access.groups` - the `agent_<name>`
   entitlement group, not a business role; otherwise it renders disabled
   ("coming soon" or "not authorized"). See
...
2. **The Tekos chat UI** (`GET /tekos`, `POST /api/chat`) - the one agent
   ...
   token when the browser asks for it (see "Streaming chat" below).
...
React tree into. Per-deployment/per-request values can't be baked into the
static JS bundle at `npm run build` time, so they're injected into the
HTML Go renders instead.
...
Go binary each time (not wired up to the BFF's dev proxy; this
component's dev loop normally goes through the full `go run .` server).
...
- `event: start` - `{"request_id": "..."}`, the first frame, carrying the
  ...
  present) for cross-service log correlation.
...
the Agent Runtime (see each hop's own `proxySSE`/`ChatStream` comments
for exactly how).
...
- **OIDC (`internal/oidc`)**: hand-rolled Authorization Code + PKCE flow
  ...
  instead of `golang.org/x/oauth2` + `github.com/coreos/go-oidc`. The
  flow (RFC 6749 §4.1, RFC 7636, OIDC Core §3.1) is small enough to
  implement directly and keep fully auditable within this component's own
  small dependency surface. **v1 hardening recommendation**: switch to
  `coreos/go-oidc` for stricter claim validation (`nonce` replay checks,
  clock-skew tolerance, discovery
...
| `WEB_DIST_DIR` | no (default `web/dist`, set to `/app/web/dist` in the image) | Vite build output directory - see `internal/assets` |
...
| `OIDC_CLIENT_SECRET` | **yes** | From an `ExternalSecret`, never hardcoded |
...
| `SESSION_HMAC_SECRET` | **yes** | Signs the opaque session-ID cookie; from an `ExternalSecret` |
| `SESSION_ENCRYPTION_KEY` | **yes** | 32 bytes, base64-encoded (AES-256); encrypts session records at rest in Redis; from an `ExternalSecret` |
| `REDIS_ADDR` | no (default `zuno-redis-master.zuno-auth.svc.cluster.local:6379`) | Server-side session store |
| `REDIS_PASSWORD` | **yes** | From an `ExternalSecret` |
| `SESSION_MAX_LIFETIME_SECONDS` | no (default `43200`, 12h) | How long a session record survives in Redis regardless of access-token refreshes |
...
| GET | `/callback` | OIDC redirect URI; exchanges the code, creates the server-side session record and sets the opaque session-ID cookie |
...
| POST | `/api/chat` | `{"session_id","message"}` → proxied to the BFF; JSON `{"reply","citations"}`, or SSE if `Accept: text/event-stream` (see above) |
| GET | `/healthz` | Liveness/readiness probe target, also used by `ansible/roles/agents/tasks/check.yml`'s smoke check |
| GET | `/static/*` | Vite-built JS/CSS assets (`web/dist`) |

## Local layout

```text
main.go                    Wiring: config, OKF load, Vite manifest, routes
internal/assets/           Vite manifest.json resolution
...
internal/reqid/             X-Zuno-Request-Id minting/propagation
...
web/                         Vite + React + TypeScript + PatternFly - see web/package.json
```
...
UBI9-minimal runtime stage copies both outputs.
