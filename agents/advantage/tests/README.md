# Advantage Contract Tests (ADR-0504)
...
(this directory) are Advantage-specific and need only repository files - no
cluster, no credentials, no model calls; **behavioral** checks stay in
`evaluations/advantage/` (ADR-0027/ADR-0028, the shared ADR-0342 runner).
...
**Blocking rule (ADR-0504):** Advantage is not exempt from this structure
requirement. `contract/`, `tasks/` and `prompts/` must contain real, green
tests before Advantage can be promoted to Stage 2 -
...
work promotes Advantage, but the requirement itself blocks promotion, not
