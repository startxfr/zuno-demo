...
- **Chart:** `gitops/charts/advantage/` — a **raw-manifest chart**
  (same shape as Tekos's). Unlike Tekos, this is legacy rather than
  deliberate: **Advantage's migration to a single `AIAgent` CR is a
  promotion-time step** (`platform/templates/agent/PROMOTION.md`
  step 2).
- **Applications:** `gitops/apps/advantage/application-d0.yaml` /
  `application-d1.yaml` (`zuno-advantage-*`), sync-wave `-99`, target
