...
def score_probe(result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Outcomes are mutually exclusive:
        ...
        false_fire   expected NO call, got one
        abstained    expected no call, got none
    """
    calls = result.get("tool_calls") or []
    expects = bool(result.get("expects_tool"))
    content = result.get("content") or ""
    mentioned = _mentions_tool(content, result.get("offered_tools") or [])
...
def score_corpus(
    ...
    *,
    ...
    positive = sum(counts.get(k, 0) for k in ("correct", "wrong_tool", "bad_args", "narrated", "missed"))
    negative = sum(counts.get(k, 0) for k in ("false_fire", "abstained"))
